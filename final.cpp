...
int main() {
    std::default_random_engine motor(std::time(nullptr)); // Inicializar el generador de números aleatorios
    ...
    while (victoriasJugador < 4 && victoriasComputadora < 4) {
        ...
        int opcionComputadora = distribution(motor);
        std::cout << "La computadora eligió " << opciones[opcionComputadora] << "." << std::endl;

        if (opcionJugador == opcionComputadora) {
            std::cout << "Empate!" << std::endl;
            empates++;
        } else if ((opcionJugador == 0 && (opcionComputadora == 2 || opcionComputadora == 3)) ||
